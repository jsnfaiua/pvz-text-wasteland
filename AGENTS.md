...
- 模块聚焦（单一职责）；保持 `docs/系统架构与设计文档.md` 描述的依赖方向。
...
- **打版本（git tag / 版本号声明 / 发布动作）必须经用户明确允许后才能执行**；
  AI 禁止擅自创建 tag、声明新版本或执行发布。日常 `git commit` 不属于版本动作，可正常进行。
- **版本 tag 数量上限 = 5 个**：最多保留最近 5 个版本 tag；每次生成新版本时，
  若已有 5 个则**删除最早的 1 个**（第 6 个生成 → 删第 1 个），保持 tag 池不超过 5 个。
  删除旧 tag 属版本管理动作，同样须在用户允许的版本流程内执行。
...
1. **禁止批量删除文件**；删除单文件须一条命令一个文件，且先取得用户显式同意。
   **例外（临时脚本，用户 2026-08-09 明确允许）**：`dev-tools/` 下的临时脚本
   （`_*.mjs` / `_*.js` / `_*.html` 等一次性调试/验证/预览脚本，含 `_cdp-*.mjs` 等）
   视为可清理项，**允许直接删除、无需逐文件征询用户同意**，但仍须遵守
   §13.9 / §13.12（一次性脚本完成后清理、不进 git）。删除时仍一条命令一个文件
   （或一次命令列出明确清单），禁止通配符批量误删正式源码。
...
8. **开发前审计 + 无损改动 + 及时止损（硬性流程）**：
   ...
   - **每次做完必查**：每个功能/修复完成即按 §9 矩阵跑对应验证 + 手工复查受影响屏幕/流程，
     确认无破坏才提交；提交信息写明验证结果。
9. **文档待办不自动等于开发任务**：`docs/`（含统一开发文档 v2.52 等）中标记 ⬜/🟡 的
   待办项仅是设计记录，**未获用户明确指示不得擅自据此开发**。用户未要求开发的功能，
   只做文档状态维护（如标记「不开发 / 搁置」），**不写代码、不动源码**。
10. **美术/渲染效果类改动：先预览、确认后再定稿（硬性流程）**：仅当改动涉及
    **美术资源渲染效果**（sprite 素材、染色/调色、像素绘制、视觉表现等需要"看效果"
    才能判断是否符合预期的改动）时，才必须走**临时浏览器测试预览**确认——用临时启动的
    浏览器实例（headless Chrome + CDP，如 `dev-tools/_cdp-*.mjs` 实测、截图到 `_qa_tmp/`）
    验证效果，**预览确认符合预期后才算定稿**。确认定稿后**删除临时浏览器资源**
    （临时 user-data-dir / CDP 实例 / 预览截图等临时产物，按 §13.12 清理），
    不得残留临时浏览器进程或数据。
    **逻辑/玩法/数值/存档/联机类改动（无渲染效果判断需求）不受此限**：不强制要求
    CDP 浏览器预览，按 §9 验证矩阵跑对应测试即可；如需演示效果可仍用临时预览。
11. **更新知识库（知识库 ≡ 记忆库，用户说「更新知识库 / 更新记忆 / 更新知识」等
    「知识库/记忆库」相关词时触发，硬性）**：**「知识库」与「记忆库」是同一套体系**——
    即 `docs/记忆库/` + 根目录 `README.md` 主索引，二者等价、同指，Agent 不得将其误解为
    两套体系。用户发出「更新知识库」类指令时，Agent 必须**自动新增一份记忆文档 + 新增一条
    索引**，无需再等用户逐条指定内容，具体流程：
    - **定位**：确定本次工作所属的功能领域（若跨多领域，可拆多篇或归到最相关的一篇）。
    - **新建文档**：在 `docs/记忆库/` 新建一篇领域记忆文档，命名中文领域名，严格遵循
      六段式结构（领域概述 → 核心技术 → 关键代码改动 → 涉及模块 → 验证方式 → 历史教训）。
      所有 `文件:函数` 引用必须真实，不确定用模块级描述兜底，禁止虚构。
    - **登记索引**：在根目录 `README.md` 的「记忆文档索引」表追加一行，`#` 连续、
      一句话概述，链接路径 `docs/记忆库/<文件名>.md`。
    - **链接校验**：跑 `node dev-tools/verify-memory-links.mjs`，确认无断链。
    - **红线**：只新增/维护 `.md`；禁止改游戏源码、既有 docs、`saves/`、`users.json`。
    - 若本次内容归属某既有领域（非新领域），则在该文档「关键代码改动」追加条目，
      并在 README 概述处更新，不新建文档。
