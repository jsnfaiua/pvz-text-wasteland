...
10. **先测试预览、确认后再定稿（硬性流程）**：任何开发（新功能/修改）必须先通过
    **临时浏览器测试预览**确认开发内容是否符合预期——用临时启动的浏览器实例
    （headless Chrome + CDP，如 `dev-tools/_cdp-*.mjs` 实测、截图到 `_qa_tmp/`）验证
    效果，**预览确认符合预期后才算定稿**；未经预览确认不得宣称完成。确认定稿后，
    **删除临时浏览器资源**（临时 user-data-dir / CDP 实例 / 预览截图等临时产物，
    按 §13.12 清理），不得残留临时浏览器进程或数据。
