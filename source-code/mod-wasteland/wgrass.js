...
export function grassRenderGround(ctx, sv, tx, ty, x0, y0) {
  ...
  for (let sy = 0; sy < 6; sy++) for (let sx = 0; sx < 6; sx++) {
    ...
    ctx.fillRect(x0 + sx * 6, y0 + sy * 6, 6, 6);
  }
  // 3 层无缝噪声 overlay（低对比防条纹）
  ctx.globalCompositeOperation = 'overlay';
  ctx.globalAlpha = 0.4;
  drawWrap(ctx, _noiseTiles.tex, x0, y0, tx, ty);
  ctx.globalAlpha = 0.2;
  drawWrap(ctx, _noiseTiles.patch, x0, y0, tx, ty);
  ctx.globalAlpha = 0.12;
  drawWrap(ctx, _noiseTiles.tone, x0, y0, tx, ty);
