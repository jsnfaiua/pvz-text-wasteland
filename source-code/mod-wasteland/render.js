...
const MC_PAL = {
    ...
    // eyes 加暗棕变体 [50,35,25]/[45,30,22]（眼窝/脸部深色阴影）——
    // 否则这些像素距 hair[0]=[55,36,21] 更近 → 判给 hair → 眼睛被染成发色（用户反馈
    // "往南走眼睛是头发的颜色，一下变头发色一下变眼睛色"）
...
function nearestPart(r, g, b, ny = 0.5) {
    ...
    // 头部区域 shoes→hair：头部（ny<0.30）只可能是头发/脸/眼，绝不可能是鞋。
    // 任何被判给 shoes 的头部像素（如深棕发丝未命中色相约束的极端变体）→ 改判 hair，
    // 染成玩家发色，避免"头顶棕色散块"。
    if (best === 'shoes' && ny < 0.30) return 'hair';
    ...
    // 注意：只作用于"真正头发区"（ny<0.30）——躯干上衣在 ny 0.35-0.65，
    // 若误用 ny<0.35 会把袖子/肩部（ny 0.35-0.45）判给 hair → "袖子横截被头发影响"（用户反馈）。
    if (best === 'shirt' && ny < 0.30) return 'hair';
