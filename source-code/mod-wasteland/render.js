...
const MC_PAL = {
    ...
    hair:  [[55, 36, 21], [74, 46, 22], [100, 66, 31], [56, 40, 24], [126, 82, 43], [120, 81, 42]],
...
function nearestPart(r, g, b, ny = 0.5) {
    ...
    // 头部区域（ny<0.42）内，任何"暖棕发色系"像素（R>G>B，R 55-170，G 38-115，B 8-70）
    ...
    // 用色相+位置双重约束一网打尽。皮肤色（[247,205,155] R/G/B 都亮）与衣色（[18,43,26] G>R）不受影响。
    if (ny < 0.42 && r > g && g > b && r >= 55 && r <= 170 && g >= 38 && g <= 115 && b >= 8 && b <= 70) {
        return 'hair';
    }
    // 头部区域 shoes→hair：头部（ny<0.42）只可能是头发/脸/眼，绝不可能是鞋。
    // 任何被判给 shoes 的头部像素（如深棕发丝未命中色相约束的极端变体）→ 改判 hair，
    // 染成玩家发色，避免"头顶棕色散块"。
    if (best === 'shoes' && ny < 0.42) return 'hair';
    // 头部区域 shirt→hair：sprite-side/walk-side 头部有 [42,78,38] 等"绿色阴影/肩部透出"
    // 像素，距 MC_PAL.shirt [18,43,26] d²=2185 较近 → 染成玩家绿衣色（[57,217,135]） →
    // 头发位置出现大块亮绿色块（用户反馈"头上散落色块"）。
    // 注意：只作用于"真正头部"（ny<0.35）——躯干上衣在 ny 0.35-0.65，
    // 若误用 ny<0.42 会把上衣上半（ny 0.35-0.42）判给 hair → "发色影响上衣上半"（用户反馈）。
    if (best === 'shirt' && ny < 0.35) return 'hair';
...
function drawWeatherParticles(ctx, sv, W, H, camX, camY) {
    ...
    if (wx.particles === 1) {   // 雨：斜线下落（侧风），世界坐标；速度/长度/粗细随强度
    ...
    } else if (wx.particles === 2) {   // 雪：慢速飘落小点 + 左右摇摆；速度/大小随强度
    ...
    } else if (wx.particles === 4) {   // 沙尘：横向快速飞沙（沙尘暴）；速度/长度/粗细随强度
        ctx.strokeStyle = 'rgba(205,175,115,0.5)';
        ctx.lineWidth = 0.8 + level * 0.2;   // 飞沙粗细随强度（扬沙 0.8 / 强沙暴 1.6）
        for (let i = 0; i < n; i++) {
            const p = parts[i];
            if (p.kind !== partKind || !p.spd) {
                const c = colsOf(i, n, W, H);
...
function drawStatusHUD(ctx, sv, W, districtNameOverride) {
    ...
    if (sv._weather && wxInfo(sv._weather).particles !== 0) {
        ...
        ctx.fillText(` · ${wxT.name}`, 232 + ctx.measureText(`第 ${sv.day} 天  ${hh}:${mm}  [${dName}]`).width + 6, 17);
    }
