// _cdp-grass-5styles.mjs — 5 个 A 衍生草样式验证
...
async function main() {
    ...
    console.log('5 列渲染:', chk.result && chk.result.value);
    ...
    const big = await send('Runtime.evaluate', { expression: `(() => {
        const cv = document.createElement('canvas'); cv.width = 216*5; cv.height = 144;
        const c = cv.getContext('2d');
        for (const [i,k] of ['A1','A2','A3','A4','A5'].entries()) c.drawImage(document.getElementById('g'+k), i*216, 0);
        return cv.toDataURL('image/png').slice(22);
    })()` });
    if (big.result && big.result.value) fs.writeFileSync('dev-tools/_cdp-grass-5styles.png', Buffer.from(big.result.value, 'base64'));
    console.log('已导出 5 列对比图');
