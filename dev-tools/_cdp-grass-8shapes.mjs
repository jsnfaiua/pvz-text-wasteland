...
async function main() {
    ...
    const chk = await send('Runtime.evaluate', { expression: `(() => {
        ...
        for (let k = 0; k < 8; k++) {
            ...
            if (!cv) { out.shapes.push('MISSING'); continue; }
            const d = cv.getContext('2d').getImageData(0, 0, 72, 48).data;
            let cnt = 0;
            for (let i = 0; i < d.length; i += 4) if (d[i] > 5 || d[i+1] > 5) cnt++;
            out.shapes.push((cnt / (72*48)).toFixed(2));
        }
        return JSON.stringify(out);
    })()` });
    console.log('渲染+8形态:', chk.result && chk.result.value);
    ...
    const big = await send('Runtime.evaluate', { expression: `(() => {
        const cv = document.createElement('canvas'); cv.width = 72*8+60; cv.height = 48;
        const c = cv.getContext('2d');
        for (let k = 0; k < 8; k++) c.drawImage(document.getElementById('shape'+k), k*102, 0);
        return cv.toDataURL('image/png').slice(22);
    })()` });
    if (big.result && big.result.value) fs.writeFileSync('dev-tools/_cdp-grass-8shapes.png', Buffer.from(big.result.value, 'base64'));
    console.log('已导出 8 形态图');
